				     ...
				     bool bSeekOnly)
{
  ...
  if (bHardLinkSupport & !bSeekOnly)
      ...
      if (!NativeDeleteFile(hFile))
	{
	  WErrMsgA errmsg;
	  oem_printf(stderr,
		     "strarc: Cannot remove temporary file '%2!.*ws!'. This "
  ...
  if (hFile == INVALID_HANDLE_VALUE)
    {
      WErrMsgA errmsg;
      oem_printf(stderr,
		 "strarc: Cannot create '%2!.*ws!' for copying: %1%%n",
		 errmsg,
		 Target->Length >> 1, Target->Buffer);
      return true;
    }
  ...
  if (!BackupCopyFile(hSource, hFile, &LinkName, Target))
      ...
      if (!NativeDeleteFile(hFile))
	{
	  WErrMsgA errmsg;
	  oem_printf(stderr,
		     "strarc: Cannot remove temporary file '%2!.*ws!': %1%%n",
		     errmsg,
		     Target->Length >> 1, Target->Buffer);
	}
	...
	if (!NativeDeleteFile(hFile))
	  {
	    WErrMsgA errmsg;
	    oem_printf(stderr,
		       "strarc: Cannot remove temporary file '%2!.*ws!': "
		       "%1%%n",
		       errmsg,
		  ...
		  if (!NativeDeleteFile(hStream))
		    {
		      ...
		      oem_printf
			(stderr,
			 "Error removing temporary stream '%2!.*ws!': "
			 "%1%%n",
			 errmsg,
			 complete_name.Length >> 1,
			 complete_name.Buffer);
		  ...
		  if (!NativeDeleteFile(hStream))
		    {
		      ...
		      oem_printf
			(stderr,
			 "Error removing temporary stream '%2!.*ws!': %1%%n",
			 errmsg,
			 complete_name.Length >> 1,
			 complete_name.Buffer);
		    }
		    ...
		    PUNICODE_STRING ShortName)
{
  ...
  if (bVerbose)
    {
      oem_printf(stderr, "%3!.*ws!, attr=%1 (%2!#x!)",
