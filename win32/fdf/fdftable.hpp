...
struct FILE_RECORD
{
  ...
  bool CalculateChecksum(DWORD dwSkipSize)
  {
    ...
    if (dwSkipSize > 0)
      if (!SetFilePointer(h, dwSkipSize, NULL, FILE_BEGIN))
	return false;

    bChecksumCalculated = GetFileCheckSum(h, &Checksum0, &Checksum1) == TRUE;
    CloseHandle(h);
...
public:
   ...
   * make sure.
   */
  FILE_RECORD *FindEqualOrAdd(LPCSTR szFilePath, DWORD dwSkipSize)
  {
    ...
    if (dwSkipSize > 0)
      if (!SetFilePointer(hFile, dwSkipSize, NULL, FILE_BEGIN))
	return NULL;
	...
	if ((fr_chelem->record->dwVolumeSerial == info.dwVolumeSerialNumber))
	  {
	    ...
	    if (!fr_chelem->record->bChecksumCalculated)
	      if (!fr_chelem->record->CalculateChecksum(dwSkipSize))
		{
		  Delete(fr_chelem);
		  continue;
		}
